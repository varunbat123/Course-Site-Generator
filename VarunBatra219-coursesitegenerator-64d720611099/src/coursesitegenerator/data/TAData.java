...
public class TAData implements AppDataComponent {
    ...
    public TAData(CourseSiteGeneratorApp initApp) {
        ...
        courseData = new CourseInfoData(app);
    ...
    public void setCellProperty(int col, int row, StringProperty prop) {
      ...
      public String setCellKeyToDifferent(String cellKey, int changeRow) {
          // We want to move all the office hours up because we changed the start time
          // so get the current row and column
          String [] row = cellKey.split("_");
          // change the row by new hour - start hour times 2 because we are in half an hour intervals
         int newRow= Integer.parseInt(row[1])+ changeRow*2;
         // the column is always the same
         int col = Integer.parseInt(row[0]);
         // return the new cellkey
        cellKey = getCellKey(col, newRow);
       return cellKey;
    } 
    ...
    return cellText;
       ...
       public void setCellValue(String cellKey, String value) {
      if(officeHours.containsKey(cellKey)){
        StringProperty cellProp = officeHours.get(cellKey);
        cellProp.setValue(value);
      }
           
       }
      public StringProperty getCell(String cellKey){
            StringProperty cellProp = officeHours.get(cellKey);
            return cellProp;
      }
    ...
    public CourseInfoData getCourseData() {
         return this.courseData;
    }
    public RecitationData getRecitationData() {
         return this.recitationData;
    }
